...
{
    int result = 0;
    readBuf[0] = PRU_EDMA_PWMSS_LIB_CMD_ID;
    readBuf[1] = PRU_EDMA_PWMSS_LIB_CMD_START;
    readBuf[2] = pwmssBits;
...
void stopPwmss(pollfd* pollfds, uint8_t pwmssBits)
{
    int result = 0;
    readBuf[0] = PRU_EDMA_PWMSS_LIB_CMD_ID;
    readBuf[1] = PRU_EDMA_PWMSS_LIB_CMD_STOP;
    readBuf[2] = pwmssBits;
    result = write(pollfds[0].fd, readBuf, 3);
    if (result > 0)
    {
...
void setData(pollfd* pollfds, uint8_t pwmssId, uint16_t period, uint16_t duA, uint16_t duB)
{
    int result = 0;
    readBuf[0] = PRU_EDMA_PWMSS_LIB_CMD_ID;
    readBuf[1] = PRU_EDMA_PWMSS_LIB_CMD_SET_DATA;
    readBuf[2] = pwmssId;
...
void setDuty(pollfd* pollfds, uint8_t pwmssId, uint16_t duA, uint16_t duB)
{
    int result = 0;
    readBuf[0] = PRU_EDMA_PWMSS_LIB_CMD_ID;
    readBuf[1] = PRU_EDMA_PWMSS_LIB_CMD_SET_DATA;
    ...
    data[0]=duA;
    data[1]=duB;
    result = write(pollfds[0].fd, readBuf, 7);
    if (result > 0)
    {
        printf("Setting PWMSS %d ...\n", pwmssId);
    }
...
void setDuty2(pollfd* pollfds,
              uint8_t pwmssId1, uint16_t duA1, uint16_t duB1,
              uint8_t pwmssId2, uint16_t duA2, uint16_t duB2)
{
    int result = 0;
    readBuf[0] = PRU_EDMA_PWMSS_LIB_CMD_ID;
    readBuf[1] = PRU_EDMA_PWMSS_LIB_CMD_SET_DATA;
    ...
    data2[1]=duA2;
    data2[2]=duB2;
    result = write(pollfds[0].fd, readBuf, 12);
    if (result > 0)
    {
        printf("Setting PWMSS ...\n");
    }
...
void setDuty3(pollfd* pollfds,
              uint8_t pwmssId1, uint16_t duA1, uint16_t duB1,
              uint8_t pwmssId2, uint16_t duA2, uint16_t duB2,
              uint8_t pwmssId3, uint16_t duA3, uint16_t duB3
              )
{
    int result = 0;
    readBuf[0] = PRU_EDMA_PWMSS_LIB_CMD_ID;
    ...
    uint16_t* data1 = (uint16_t*)(readBuf+3);
    uint16_t* data2 = (uint16_t*)(readBuf+8);
    uint16_t* data3 = (uint16_t*)(readBuf+13);
    data1[0]=duA1;
    data1[1]=duB1;
    data2[0]=duA2;
    data2[1]=duB2;
    data3[0]=duA3;
    data3[1]=duB3;
    result = write(pollfds[0].fd, readBuf, 17);
    if (result > 0)
    {
        printf("Setting PWMSS ...\n");
    }
...
void executeSetDuty(pollfd* pollfds, uint8_t pwmssId, uint16_t duA, uint16_t duB)
{
    printf("cmpA: %d, cmpB: %d\n", duA, duB);
    if(pwmssId > 2) {
        printf("Usage: pru-pwmss-arm set-duty <pwmss id><cmp A><cmp B>");
        return;
    }
    setDuty(pollfds, pwmssId, duA, duB);
}
void executeSetDuty2(pollfd* pollfds, uint8_t pwmssId1, uint16_t duA1, uint16_t duB1, uint8_t pwmssId2, uint16_t duA2, uint16_t duB2)
{
    if(pwmssId1 > 2 || pwmssId2 > 2 ) {
        printf("Usage: pru-pwmss-arm set-duty <pwmss id><cmp A><cmp B>");
        return;
    }
    setDuty2(pollfds, pwmssId1, duA1, duB1, pwmssId2, duA2, duB2);
}

void executeSetDuty3(pollfd* pollfds, uint8_t pwmssId1, uint16_t duA1, uint16_t duB1,
                                      uint8_t pwmssId2, uint16_t duA2, uint16_t duB2,
                                      uint8_t pwmssId3, uint16_t duA3, uint16_t duB3)
{
    if(pwmssId1 > 2 || pwmssId2 > 2 || pwmssId3 > 2 ) {
    ...
    setDuty3(pollfds, pwmssId1, duA1, duB1, pwmssId2, duA2, duB2, pwmssId3, duA3, duB3);
}
...
int main(int argc, char *argv[])
{
    ...
    else
    {
        ...
        else if ((strcmp("set-duty", argv[1]) == 0) && (argc == 5))
        {
            const char* devNumStr = argv[2];
            const char* duAStr = argv[3];
            const char* duBStr = argv[4];
            executeSetDuty(pollfds, atoi(devNumStr), atoi(duAStr), atoi(duBStr));
        }
        else if ((strcmp("set-duty", argv[1]) == 0) && (argc == 8))
        {
            const char* devNumStr1 = argv[2];
            const char* duAStr1 = argv[3];
            const char* duBStr1 = argv[4];

            const char* devNumStr2 = argv[5];
            const char* duAStr2 = argv[6];
            const char* duBStr2 = argv[7];
            executeSetDuty2(pollfds, atoi(devNumStr1), atoi(duAStr1), atoi(duBStr1),
                                     atoi(devNumStr2), atoi(duAStr2), atoi(duBStr2));
        }
        else if ((strcmp("set-duty", argv[1]) == 0) && (argc == 11))
        {
            const char* devNumStr1 = argv[2];
            const char* duAStr1 = argv[3];
            const char* duBStr1 = argv[4];
            ...
            const char* devNumStr3 = argv[8];
            const char* duAStr3 = argv[9];
            const char* duBStr3 = argv[10];
            executeSetDuty3(pollfds, atoi(devNumStr1), atoi(duAStr1), atoi(duBStr1),
                                     atoi(devNumStr2), atoi(duAStr2), atoi(duBStr2),
                                     atoi(devNumStr3), atoi(duAStr3), atoi(duBStr3));
        }
        else
        {
